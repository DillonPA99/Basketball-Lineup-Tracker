...
try:
    ...
    st.warning("Firebase not available. Running in local mode only.")
...
def generate_csv_report():
    """Generate CSV report as fallback when Excel is not available"""
    output = io.StringIO()
    
    # Game Summary
    output.write("Basketball Game Report\n")
    output.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    output.write(f"Final Score: Home {st.session_state.home_score} - Away {st.session_state.away_score}\n")
    output.write(f"Quarter: {st.session_state.current_quarter}\n")
    output.write(f"Game Clock: {st.session_state.current_game_time}\n\n")
    
    # Team Roster
    if st.session_state.roster:
        output.write("TEAM ROSTER\n")
        output.write("Player Name,Jersey Number,Position\n")
        for player in st.session_state.roster:
            output.write(f"{player['name']},{player['jersey']},{player.get('position', 'N/A')}\n")
        output.write("\n")
    
    # Score History
    if st.session_state.score_history:
        output.write("SCORE HISTORY\n")
        output.write("Team,Points,Quarter,Game Time,Scorer,Shot Type\n")
        for score in st.session_state.score_history:
            output.write(f"{score.get('team', '')},{score.get('points', 0)},{score.get('quarter', '')},{score.get('game_time', '')},{score.get('scorer', '')},{score.get('shot_type', '')}\n")
...
Please see attached file for complete game statistics and analytics.
...
def render_enhanced_sidebar():
    ...
    with st.sidebar:
        ...
        if not has_game_data:
            ...
            report_button_text = "📊 Download Excel Report" if EXCEL_AVAILABLE else "📊 Download CSV Report"
            if st.button(report_button_text, type="primary"):
                try:
                    if EXCEL_AVAILABLE:
                        report_buffer = generate_game_report_excel()
                        filename = f"basketball_game_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                        mime_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                        file_label = "⬇️ Download Excel File"
                    else:
                        report_buffer = generate_csv_report()
                        filename = f"basketball_game_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                        mime_type = "text/csv"
                        file_label = "⬇️ Download CSV File"
                    ...
                    st.download_button(
                        ...
                        file_name=filename,
                        mime=mime_type,
                        help=f"Click to download the complete game report as a {'Excel' if EXCEL_AVAILABLE else 'CSV'} file"
                    )
                    
                    success_msg = "✅ Excel report generated!" if EXCEL_AVAILABLE else "✅ CSV report generated!"
            ...
            if not EXCEL_AVAILABLE:
                st.info("💡 Install xlsxwriter for Excel support: `pip install xlsxwriter`")
            ...
            if st.button("📧 Prepare Email Content"):
                try:
                    ...
                    st.info("💡 Attach the report file to your email!")
            ...
            with st.expander("📖 How to Email Report"):
                ...
                **Steps to email your game report:**
                ...
                1. Click "📊 Download {'Excel' if EXCEL_AVAILABLE else 'CSV'} Report" to get the data file
                ...
                6. Attach the {'Excel' if EXCEL_AVAILABLE else 'CSV'} file you downloaded
                ...
                **Note:** {'Excel format available' if EXCEL_AVAILABLE else 'CSV format (install xlsxwriter for Excel)'}
                """)
